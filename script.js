...
const FEATURES = [
  {
    ...
    title: "Adaptive interview", body: "A conversational engine that probes your skills, constraints, and ambitions — not a static form."
  },
  {
    icon: '<circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/>',
    title: "Skill-aware roadmap", body: "Milestones calibrated to what you already know and the gaps worth closing."
  },
  {
    icon: '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14a9 3 0 0 0 18 0V5"/><path d="M3 12a9 3 0 0 0 18 0"/>',
    title: "Schema + APIs", body: "Generated database schema, REST/GraphQL surface, and auth model — ready to scaffold."
  },
  {
    icon: '<path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>',
    title: "Stack rationale", body: "Every framework, library, and service picked with a written tradeoff, not a vibe."
  },
  {
    ...
    title: "Deployment plan", body: "Hosting, CI/CD, environments, and observability mapped to your team size and budget."
  },
  {
    icon: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',
    title: "Resume impact", body: "A recruiter-grade summary of what the project demonstrates about you."
  },
];
...
const FAQ = [
  { q: "Who is Architect AI for?", a: "Engineers, students, and indie builders who want a credible architecture for a new project — without spending a weekend whiteboarding it alone." },
  { q: "Does it write code?", a: "Not yet. The first release generates a complete blueprint: stack, schema, APIs, roadmap, and deployment plan. Scaffolding lands in a later phase." },
  { q: "What AI model powers the interview?", a: "A Dify workflow orchestrates the multi-step reasoning. The model layer is swappable without changing the product surface." },
  { q: "Can I export the blueprint?", a: "Yes — Markdown, PDF, and a shareable link are planned for the blueprint dashboard." },
];

// Interview script
const QUESTIONS = [
  {
    id: "idea", bot: "Hey 👋 I'm Architect. In one line, what are you building?",
    options: ["A SaaS dashboard", "A social mobile app", "An AI tool", "An e-commerce site"],
    blueprintLabel: "Project idea"
  },
  {
    id: "users", bot: "Got it. Who is it for?",
    options: ["Developers", "Small businesses", "Consumers", "Enterprise teams"],
    blueprintLabel: "Target users"
  },
  {
    id: "skill", bot: "What's your strongest stack right now?",
    options: ["React + Node", "Next.js + Postgres", "Python + FastAPI", "Beginner — pick for me"],
    blueprintLabel: "Primary stack"
  },
  {
    id: "scope", bot: "Last one — how soon do you want a v1 shipped?",
...
document.querySelectorAll('a[href^="#"]').forEach(a => {
  a.addEventListener("click", e => {
    ...
    if (id && id.length > 1) { e.preventDefault(); scrollToHash(id); }
  });
});
...
(() => {
  ...
  function step() {
    ...
    for (const p of particles) {
      ...
      ctx.fillStyle = "rgba(139,148,255,.55)"; ctx.fill();
    }
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (d < 130) {
          ...
          ctx.strokeStyle = `rgba(99,102,241,${(1 - d / 130) * .18})`; ctx.lineWidth = 1; ctx.stroke();
        }
      }
    }
...
function renderOptions(opts) {
  ...
  optionsEl.querySelectorAll(".option-chip").forEach(b => b.addEventListener("click", () => submitAnswer(b.textContent)));
}
function setProgress() {
  fillEl.style.width = `${(qIndex / QUESTIONS.length) * 100}%`;
  progLabel.textContent = qIndex >= QUESTIONS.length ? "Complete" : `Question ${qIndex + 1} / ${QUESTIONS.length}`;
}
function appendBlueprintBlock(label, value) {
  if (blueprintEl.querySelector(".bp-empty")) blueprintEl.innerHTML = "";
  const block = document.createElement("div");
  block.className = "bp-block";
  block.innerHTML = `<div class="bp-label">${label}</div><div class="bp-content">${value}</div>`;
...
function finishInterview() {
  appendBlueprintBlock("Recommended stack", recommendStack(answers));
  setTimeout(() => appendBlueprintBlock("Roadmap (v1)",
    `Week 1 — schema + auth\nWeek 2 — core CRUD + UI shell\nWeek 3 — polish + deploy`), 400);
  setTimeout(() => appendBlueprintBlock("Resume impact",
    `Shipped a full-stack ${(answers.idea || "product").toLowerCase()} for ${(answers.users || "users").toLowerCase()} in ${(answers.scope || "a sprint").toLowerCase()}.`), 800);
  addMessage("Done! Your blueprint preview is on the right. Tap 'See a sample blueprint' for a full example.", "bot");
...
function submitAnswer(text) {
  ...
  qIndex++; setProgress();
  optionsEl.innerHTML = ""; inputEl.value = "";
  const typing = showTyping();
  setTimeout(() => {
    typing.remove();
    if (qIndex < QUESTIONS.length) askNext();
    else finishInterview();
  }, 700 + Math.random() * 400);
}
function askNext() {
  const q = QUESTIONS[qIndex];
  addMessage(q.bot, "bot");
  renderOptions(q.options);
  setProgress();
}
function restart() {
  qIndex = 0; for (const k of Object.keys(answers)) delete answers[k];
  messagesEl.innerHTML = ""; blueprintEl.innerHTML = `
    <div class="bp-empty">
...
formEl.addEventListener("submit", e => { e.preventDefault(); submitAnswer(inputEl.value); });

// Kick off the interview the first time the demo enters view
const demoIO = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting && qIndex === 0 && messagesEl.children.length === 0) {
      askNext(); demoIO.disconnect();
    }
  });
}, { threshold: 0.3 });
